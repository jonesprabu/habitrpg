...
habitrpg.controller("GroupsCtrl", ['$scope', '$rootScope', 'Shared', 'Groups', '$http', '$q', 'User', 'Members', '$state', 'Notification',
    ...
    $scope.postChat = function(group, message){
      ...
      Groups.Group.postChat({gid: group._id, message:message, previousMsg: previousMsg}, undefined, function(data){
        ...
        if (group.privacy == 'public'){
          Analytics.track('behavior','Group Chat',{'groupType':group.type,'privacy':group.privacy,'groupName':group.name,'message':message})
        } else {
          Analytics.track('behavior','Group Chat',{'groupType':group.type,'privacy':group.privacy})
        }
      }, function(err){
        $scope._sending = false;
      });
    ...
    function($scope, Groups, User, Challenges, $rootScope, $state, $location, $compile) {
      ...
      $scope.create = function(group){
        ...
        if (confirm(window.env.t('confirmGuild'))) {
          group.$save(function(saved){
            if (saved.privacy == 'public') {Analytics.track('behavior','Join Group',{'owner':true,'groupType':'guild','privacy':saved.privacy,'groupName':saved.name})}
            else {Analytics.track('behavior','Join Group',{'owner':true,'groupType':'guild','privacy':saved.privacy})}
      ...
      $scope.join = function(group){
        ...
        group.$join(function(joined){
          if (joined.privacy == 'public') {Analytics.track('behavior','Join Group',{'owner':false,'groupType':'guild','privacy':joined.privacy,'groupName':joined.name})}
          else {Analytics.track('behavior','Join Group',{'owner':false,'groupType':'guild','privacy':joined.privacy})}
  ...
  .controller("PartyCtrl", ['$rootScope','$scope', 'Groups', 'User', 'Challenges', '$state', '$compile',
    function($rootScope,$scope, Groups, User, Challenges, $state, $compile) {
      ...
      $scope.create = function(group){
        group.$save(function(){
          Analytics.track('behavior','Join Group',{'owner':true,'groupType':'party','privacy':'private'});
      ...
      $scope.join = function(party){
        ...
        group.$join(function(){
          Analytics.track('behavior','Join Group',{'owner':false,'groupType':'party','privacy':'private'});
          $rootScope.hardRedirect('/#/options/groups/party');
        });
      }
